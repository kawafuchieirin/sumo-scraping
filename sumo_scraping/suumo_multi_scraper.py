...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('multi_scraper.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SuumoMultiScraper:
...
使用例:
  # 渋谷駅から100件取得
  python suumo_multi_scraper.py --stations 渋谷 --count 100
  
  # 渋谷、新宿、池袋から合計200件取得
  python suumo_multi_scraper.py --stations 渋谷 新宿 池袋 --count 200
  
  # 山手線全駅から500件取得
  python suumo_multi_scraper.py --yamanote --count 500
  
  # 対応駅一覧を表示
  python suumo_multi_scraper.py --list-stations
        """
    )
...
async def main():
    ...
    scraper = SuumoMultiScraper(
        stations=stations,
        target_count=args.count,
        prefecture=args.prefecture
    )
    
    logger.info(f"Starting SUUMO Multi-Station scraper")
